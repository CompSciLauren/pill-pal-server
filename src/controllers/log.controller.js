...
const LogEntry = require("../models/log.model.js");
...
exports.create = (req, res) => {
    ...
    console.log("%s,%s,%s,%s", newLog.User_Email, newLog.Date, newLog.Height, newLog.Weight);
    ...
    LogEntry.getLog(req.params.email, req.params.Date,  (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            ...
            res.status(500).send({
              message: "Error retrieving Log with email " + req.params.email + "and date" + req.params.date
            });
          }
        } else res.send(data);
      });
...
exports.update = (req, res) => {
    ...
    LogEntry.update(req.params.email, req.params.date,
      ...
      (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            ...
            res.status(500).send({
              message: "Error updating Log with email " + req.params.email + "and date" + req.params.date
            });
          }
        } else res.send(data);
      }
